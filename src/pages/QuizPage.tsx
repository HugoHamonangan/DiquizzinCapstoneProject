        ...
        <>
          <div className="max-w-5xl mx-auto flex flex-col justify-center items-center mt-10 space-y-2 mb-[50px]">
                      ...
                      <div>
                        ...
                        {unansweredQuestions.map((num, index) => (
                          <React.Fragment key={num}>
                            {index > 0 && ', '}
                            <a href={`#question-${num}`} className="text-blue-500 underline">
                              {num}
                            </a>
                          </React.Fragment>
                        ))}
            ...
            {questionsAndAnswers.length > 0 && !showResult ? (
              <button onClick={submitAnswers} className="bg-[#f9a826] hover:bg-yellow-600 font-bold text-white rounded-md mt-11 px-6 py-3 w-fit">
                Submit
              </button>
            ) : null}
